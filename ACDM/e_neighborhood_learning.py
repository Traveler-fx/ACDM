...
from ACDM.c_neighborhood_initialization import connections_cal, neighborhood_initialization, interaction_process
...
def neighborhood_learning(skeleton, data, predict_labels, neighborhood,neighborhood_r,neighborhood_r_behind, k_nearest_neighbors, count, order,real_labels, record, n,k,l):
    ...
    while (True):
        ...
        print("Iteration:",iter, "Annotation:",count, "ARI Score:",ari)
        iter=iter+1
        if flag == True:
            break
    return record
...
def NNGAMD(data, real_labels,m,k,l,n):
    k_nearest_neighbors = k_nearest_neighbor_cal(data, k)
    skeleton, representative = graph_initialization(data)
    predict_labels=initialization_cut(skeleton, m, representative)
    record = [[{"iter": 0, "interaction": 0, "ari": adjusted_rand_score(real_labels,predict_labels)}]]
    print("Iteration:",0, "Annotation:",0, "ARI Score:",adjusted_rand_score(real_labels,predict_labels))
    ...
    print("Iteration:",1, "Annotation:",count, "ARI Score:",adjusted_rand_score(real_labels, predict_labels))
