...
def generate_dermatology_data(path):
    df=pd.read_csv(path, header=None)
    df_labels=df.iloc[:,34]
    df_data=df.iloc[:,0:34]
    #将问号用np.nan代替
    ...
    data = np.array(df_data).astype(float)
...
if __name__ == '__main__':
    data, labels = generate_dermatology_data(path="dermatology.data")
    print(np.shape(data))
